...
function init(P::InfluenzaParameters)
    [Human(i) for i = 1:P.grid_size_human]
end
...
function main(simnum::Int64, P::InfluenzaParameters)
    ...
    for t=1:P.sim_time        
        ...
        latent_ctr[t], symp_ctr[t], asymp_ctr[t], latent_nvac_ctr[t], latent_vac_ctr[t] = update_human(humans,P)

        if t%7 == 0
            for i = 1:P.grid_size_human
                humans[i].vaccineEfficacy = humans[i].vaccineEfficacy-P.waning_rate
                if humans[i].vaccineEfficacy < 0
                    hsum(vaccine1)umans[i].vaccineEfficacy = 0.0
                end
            end
        end
    ...
    for i = 1:length(humans)        
        ...
        vax_status[i] = humans[i].vaccination_status
    ...
    vax_status, SympOrNot, demographic_group,latent_nvac_ctr,latent_vac_ctr
end

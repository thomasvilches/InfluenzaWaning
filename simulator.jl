...
create_fn(ar, ve, wr) = "ar_$(replace(string(ar), "." => "_"))_ve_$(replace(string(ve), "." => "_"))_wr_$(replace(string(wr), "." => "_"))"
...
function run_attackrates(ARS, VES, WR)
    ## runs the specified number of simulations for beta (or a range of betas) 
    ...
    for ar in ARS, ve in VES, wr in WR       
        β = f(ar)         
        @everywhere P = InfluenzaParameters(sim_time = 250, vaccine_efficacy = $ve, transmission_beta=$β, waning_rate = $wr)          
        results = pmap(x -> main(x, P), 1:NUMOFSIMS)
        dname = "$RF/$(create_fn(ar, ve, wr))"
        dataprocess(results, P, fileappend=dname)
        create_RES([1,2,3,4,5], RF, ar=ar, ve=ve,wr=wr, resultformat="arve");
        create_RES([1], RF, ar=ar, ve=ve,wr=wr, resultformat="arve");
        create_RES([2], RF, ar=ar, ve=ve,wr=wr, resultformat="arve");
        create_RES([3], RF, ar=ar, ve=ve,wr=wr, resultformat="arve");
        create_RES([4], RF, ar=ar, ve=ve,wr=wr, resultformat="arve");
        create_RES([5], RF, ar=ar, ve=ve,wr=wr, resultformat="arve");
...
function dataprocess(results, P::InfluenzaParameters; fileappend="./")     
    ...
    for i=1:NUMOFSIMS
        ...
        resultsLNV[:,i] = results[i][16]
        resultsLV[:,i] = results[i][17]
    end
    ...
    writedlm(string("$fileappend", "_NVacLat.dat"),resultsLNV)
    writedlm(string("$fileappend", "_VacLat.dat"),resultsLV)
    JSON.print(open(string("$fileappend", "_parameters.dat"), "w"), P, 4)
end


function create_RES(agegroup, folder; ar=0.0, ve=0.0, wr=0.0, β=0.0,  resultformat="arve")    
    ...
    if resultformat == "arve"
        cfn = "$folder/$(create_fn(ar, ve, wr))"
        if length(agegroup) == 1  ## only dealing with one agegroup
            resname = "$folder/RES_$(create_fn(ar, ve, wr))_ag_$(agegroup[1]).dat"
        else 
            resname = "$folder/RES_$(create_fn(ar, ve, wr))_ag_0.dat"
        end  
    else 

mutable struct Human{T <: Number} ## mutable structs are stored on the heap 
    ...
    vaccination_status::Int64

    Human{Int64}() = new(-1, SUSC, UNDEF, 0, 999, 0.0, -1, UNDEF,
                            nothing, nothing, nothing, 0, 0.0, 0, 0)
    function Human(idx)
        h = Human{Int64}()
        h.index = idx
        return h
...
function update_human(h, P::InfluenzaParameters)
    ...
    return n1, n2, n3, nl_nvac, nl_vac #corresponds to latent, symp, asymp
end
